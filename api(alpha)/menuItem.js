...
router.post('/api/menu_items', file, async (req, res) => {
    ...
    if(isAllowed != "") return res.status(400).send({error: `Invalid field(s) ${isAllowed} in request`})
    
    if(req_body.includes('menu_id')){
        const isArr = Object.prototype.toString.call(req.body['menu_id']) == '[object Array]';
        if(!isArr) return res.status(400).send({error:'menu_id should be an array of strings'})

        const isMenu = []
        req.body['menu_id'].forEach(element => {
            const menu = Menu.findOne({_id: element})
            if(menu._doc === undefined) isMenu.push(element)
        });
        
        if(isMenu.length !== 0) return res.status(400).send({error:`There are no mathcing records for menu_id(s) ${isMenu}`})
    }
    ...
    if(req.headers['content-type'] === 'application/json' ){
        ...
        try {
            if(req_body.includes('image')) return res.status(400).send({error:'Unable to create menu item. Use multipart/form-data request to create menu item with images'})
...
router.get('/api/menu_items', async (req, res) => {
    try {
        ...
        if(!menuItems) return res.status(404).send({message: 'No menu items found'})
...
router.get('/api/menu_items/:id', async (req, res) => {
    ...
    try {
        ...
        if(!menuItem.length === 0) return res.status(404).send({message: 'The menu item does not exist'})
...
router.put('/api/menu_items/:id', async (req, res) => {
    ...
    if(!menuItem) return res.status(404).send({message:'The menu item you are trying to update does not exist'})
    res.status(200).send(menuItem)
...
router.delete('/api/menu_items/:id', async (req, res) => {
    ...
    if(!menuItem) return res.status(404).send({message: 'The menu item that you are trying to delete does not exist'})
