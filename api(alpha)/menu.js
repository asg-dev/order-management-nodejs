...
router.get('/api/menus/:id', async (req,res) => {
    try {
        ...
        if(!menu) return res.status(404).send({message: 'The menu does not exist'})
        res.status(200).send(menu)
    } catch (error) {
        console.log(error)
        res.status(500).send({error})
...
router.get('/api/menus/:id/menu_items', async (req, res) => {
    try {
        const menu = await Menu.findOne({_id: req.params.id})
        if(!menu) return res.status(404).send({message:'This menu does not exist'})
        const menuItems = await menu.populate('menu_items').execPopulate()
        if(!menuItems) return res.status(404).send({message:'There are no items added to this menu'})
...
router.put('/api/menus/:id/add_item', async(req, res) => {
    try {
        ...
        if(menuItem === null) return res.status(404).send({message:'The menu item you are trying to delete does not exist'})    
        res.status(200).send({message:'Item successfully added to the menu'})
    } catch (error) {
        res.status(500).send(error)
    }
...
router.delete('/api/menus/:id/menu_items/:item_id', async (req, res) => {
    try {
        const menuitem = await MenuItem.findOneAndUpdate({_id:req.params['item_id']}, {$unset: {menu_id:req.body['id']}}, {new: true, runValidators: true})
        if(menuitem === null) return res.status(404).send({message:'The menu item you are trying to delete does not exist'})    
...
router.post('/api/menus', async (req, res) => {
    try {
        ...
        if(!isAllowed) return res.status(400).send({error: 'Invalid field in request'})
...
router.put('/api/menus/:id', async (req, res) => {
    try {
        ...
        const menu = await Menu.findByIdAndUpdate({_id: req.params.id}, {$set:req.body}, {new: true, runValidators: true})
        if(!menu) return res.status(404).send({message:'The menu you are trying to update does not exist'})
        res.status(201).send(menu)
...
router.delete('/api/menus/:id', async (req, res) => {
    try {
        const menu = await Menu.findOne({_id: req.params.id})
        if(!menu) return res.status(404).send({error:'The menu that you are trying to delete does not exist'})
        await menu.remove()
