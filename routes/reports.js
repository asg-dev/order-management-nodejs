...
router.get('/admin/reports', async (req, res) => {
    res.render('reports', {page_name: 'Reports'})
})
...
router.get('/admin/reports/filter', async(req, res)=> {
    ...
    if (orders.length == 0) return res.render('reports', {page_name:'Reports',  start_date: start_date, end_date: end_date})
    const total =orders.filter((obj) => {return obj.total}).map(function(obj) {return obj.total;});
